...
# Helper function to clean up MCP manager and unregister process
def cleanup_mcp_manager(manager, session_id):
    """Clean up MCP manager and unregister from process tracking."""
    try:
        if manager:
            manager.stop()
        ...
        print(f"[MCP CLIENT] Warning: Error cleaning up MCP manager for session {session_id}: {cleanup_e}", flush=True)
...
def list_tools():
    ...
    try:
        ...
        try:
            ...
            cleanup_mcp_manager(mcp_manager, session_id)
...
def run_server():
    ...
    print(f"[MCP CLIENT] Architecture: Redis with process tracking (stateless, multi-worker compatible)", flush=True)
